    ...
    <div className="max-w-6xl mx-auto px-4 py-12 space-y-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <aside className="lg:col-span-3 space-y-3">
          {[
            { id: 'profile', icon: User, label: 'Informações Pessoais' },
            { id: 'security', icon: Shield, label: 'Segurança' },
            { id: 'notifications', icon: Bell, label: 'Notificações' },
            { id: 'integrations', icon: Calendar, label: 'Integrações' },
            ...
            >
              <item.icon className={`w-5 h-5 ${activeTab === item.id ? 'text-white' : 'text-stone-300'}`} />
              {item.label}
            </button>
          ))}
        </aside>
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'security' && (
              <motion.div
                ...
                className="space-y-10"
              >
                {user?.role !== 'patient' && <MfaSetup />}

                <div className="bg-white p-12 rounded-[3rem] border border-stone-100 shadow-sm space-y-8">
                  <div>
                    <h3 className="text-2xl font-serif italic text-stone-900">Alterar Senha</h3>
                    <p className="text-stone-500 font-serif italic text-lg mt-1">Mantenha sua conta segura com uma senha forte.</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      <label className="text-[10px] font-bold text-stone-400 uppercase tracking-[0.2em] ml-2">Senha Atual</label>
                      <input type="password" placeholder="••••••••" className="w-full px-8 py-5 bg-stone-50 border border-stone-100 rounded-[2rem] focus:bg-white outline-none transition-all" />
                    </div>
                    <div className="space-y-3">
                      <label className="text-[10px] font-bold text-stone-400 uppercase tracking-[0.2em] ml-2">Nova Senha</label>
                      <input type="password" placeholder="••••••••" className="w-full px-8 py-5 bg-stone-50 border border-stone-100 rounded-[2rem] focus:bg-white outline-none transition-all" />
                    </div>
                  </div>
                  <div className="flex justify-end">
                    <button className="px-10 py-4 bg-stone-800 text-white rounded-[2rem] font-bold hover:bg-stone-700 transition-all">
                      Atualizar Senha
                    </button>
                  </div>
                </div>
